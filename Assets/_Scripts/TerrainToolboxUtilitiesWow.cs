...
{
	[Serializable]
	public class UtilitySettingsWow : ScriptableObject
	{
		// Terrain Split
	...
	public class TerrainToolboxUtilitiesWow
	{
		...
		public TerrainPalette m_SelectedLayerPalette = ScriptableObject.CreateInstance<TerrainPalette>();
		// Heightmap export
		...
		// Splatmaps
		public List<Texture2D> m_Splatmaps = new List<Texture2D>();
		...
		public void AddLayersToTerrain(Terrain terrain) {
			//Terrain[] terrains;
		...
		public void AddLayersToSelectedTerrains()
		{
			Terrain[] terrains;
			/*if (m_Settings.ApplyAllTerrains)
			{
		...
		public void ExportSplatmapsToTerrain(Terrain terrain) {
			// validate settings
			...
			try {				
				for (int z = 0; z < m_Splatmaps.Count; z++) {
					if (m_Splatmaps[z] != null) {
						...
						ToolboxHelper.ResizeControlTexture(terrain.terrainData, resolution.x);
						var newPixels = m_Splatmaps[z].GetPixels(0, 0, resolution.x, resolution.y);
						Debug.Log("alphamapTextures length" + terrain.terrainData.alphamapTextures.Length + ", m_Splatmaps.Count; " + m_Splatmaps.Count);
						if (terrain.terrainData.alphamapTextures[z] != null) {
							terrain.terrainData.alphamapTextures[z].SetPixels(newPixels);
							terrain.terrainData.alphamapTextures[z].Apply();
						}
					}
				}
			}
		...
		public void ExportSplatmapsToTerrain()
		{
			// validate settings
			// all splatmaps same resolution
			// terrains same control map resolution
		...
		bool ValidateSplatmap(List<Terrain> terrains, Vector2Int resolution, int expectedCount, int tilesCount)
		{
			...
			{
				if (terrain.terrainData.alphamapTextures.Length < m_Splatmaps.Count) // m_SplatmapList.count
				{
